...
async def test_get_tron_info_db_error(override_get_db, mocker, async_client):
    mock_data = {"address": "fake_address", "balance": 100, "bandwidth": 256, "energy": 10000}

    mocker.patch("src.api.api.get_tron_account_info", AsyncMock(return_value=mock_data))
    mocker.patch("src.api.api.save_request", AsyncMock(side_effect=Exception('DB error')))
    ...
    assert response.status_code == 500
    assert response.json() == {'detail': 'Internal server error: DB error'}
